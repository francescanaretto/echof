...
FILE_PATH = Path("albert")
TARGET_COL = "class"
TEST_SIZE = 0.2
RANDOM_STATE = 42

OUT_DIR = Path("../Data-original/albert")
...
def main():
    ...
    X_train.to_csv(OUT_DIR / "train_set_albert.csv", index=False)
    X_test.to_csv(OUT_DIR / "test_set_albert.csv", index=False)
    y_train.to_csv(OUT_DIR / "train_labels_albert.csv", index=False)
    y_test.to_csv(OUT_DIR / "test_labels_albert.csv", index=False)
