...
BB_MODELS = ["nn", "rf"]           # etichette con BB=nn e BB=rf

N_SYNTH   = 80000                  # "match" => len(train_set), or an integer (e.g. 80000)
MAX_SYNTH_PER_DATASET = 80000      # cap di sicurezza
TEST_SIZE = 0.30
RANDOM_STATE = 42
BATCH_SIZE_PRED = 4096
USE_FLOAT32 = True
APPLY_SCALING = True               # match the copula pipeline: fit the scaler on real data and apply it to synthetic data
...
DELTA = 1e-5
MAX_MODEL_MB = 80
BINNER = "priv-tree"       # "uniform" / "quantile" per alternative

# generazione AIM
N_ITERS = 1000
DEGREE = 2
NUM_MARGINALS = None       # use the internal selection
MAX_CELLS = 10_000

#  INFERENZA & SANITIZZAZIONE
LOW_CARD_INT_AS_CAT = True
LOW_CARD_THRESHOLD = 30
...
def prepare_wave_multi_mst(df_raw: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    """
    Conservative preprocessing for wave-multi style datasets.

    It converts each column to a discretized categorical representation before
    passing the data to the MST pipeline, preventing unstable continuous bin
    edges on highly regular synthetic wave features.
    """
    df = df_raw.copy()
...
def main():
    ...
    for dataset in DATASETS:
        try:
            print(f"\n=== Dataset: {dataset} | Generator: MST (DP) ===")
            if dataset in ROBUST_MST_DATASETS:
                X_real = load_train_df(dataset)

                # 2) robust preparation for MST
                if dataset in {"wave-multi", "wave_multi"}:
                    X_sanit, domain = prepare_wave_multi_mst(X_real)
                else:
            ...
            try:
                ...
                if "probabilities contain NaN" in msg:
                    ...
                    print("  [warn] MST NaN on main configuration – retrying with a more conservative fallback")
                    ...
                    X_fb, dom_fb = prepare_wave_multi_mst(X_real)
                    pipeline = build_pipeline_privbayes(
                        n_bins=10,
                        degree=1,
                        n_iters=1000,
